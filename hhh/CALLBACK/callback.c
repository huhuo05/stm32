...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    if (htim == &htim7) // 100HZ
    {
        ...
        // 计算速度
        callback.PID[0].wheel_speed = (fp32)callback.cout[0][10] * p * D / 4.0 / Redu_Ratio / Line_number;
        callback.PID[1].wheel_speed = (fp32)callback.cout[1][10] * p * D / 4.0 / Redu_Ratio / Line_number;
        callback.PID[2].wheel_speed = (fp32)callback.cout[2][10] * p * D / 4.0 / Redu_Ratio / Line_number;
        callback.PID[3].wheel_speed = (fp32)callback.cout[3][10] * p * D / 4.0 / Redu_Ratio / Line_number;
